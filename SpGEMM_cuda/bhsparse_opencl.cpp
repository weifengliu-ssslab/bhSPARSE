...
int bhsparse_opencl::initPlatform()
{
    ...
    for (unsigned int i = 0; i < _numPlatforms; i++)
    {
        ...
        if (_numGpuDevices > 0)
        {
            ...
            err |= _basicCL.getDeviceInfo(_cdGpuDevices[0], _gpuDeviceName, _gpuDeviceVersion,
    ...
    // Gpu commandqueue
    if (_profiling)
        err = _basicCL.getCommandQueueProfilingEnable(&_cqGpuCommandQueue, _cxGpuContext, _cdGpuDevices[0]);
    else
        err = _basicCL.getCommandQueue(&_cqGpuCommandQueue, _cxGpuContext, _cdGpuDevices[0]);
    if(err != CL_SUCCESS) return err;
